...
import '../vehicle_details_renter_screen.dart';

class VehicleCardRenter extends StatefulWidget {
  final Vehicle vehicle;

  const VehicleCardRenter({Key? key, required this.vehicle}) : super(key: key);

  @override
  _VehicleCardState createState() => _VehicleCardState();
}

class _VehicleCardState extends State<VehicleCardRenter> {
          ...
          GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => VehicleDetailsRenterScreen(vehicle: widget.vehicle),
                ),
              );
            },
            child: Column(
